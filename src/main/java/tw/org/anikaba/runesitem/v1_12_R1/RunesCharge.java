...
public class RunesCharge extends RunesMeta implements FireworkEffectMeta {
    static final ItemMetaKey EXPLOSION = new ItemMetaKey("Explosion", "firework-effect");

    private FireworkEffect effect;

    RunesCharge(RunesMeta meta) {
        super(meta);

        if (meta instanceof RunesCharge) {
            effect = ((RunesCharge) meta).effect;
        }
    }

    RunesCharge(Map<String, Object> map) {
        super(map);

        setEffect(SerializableMeta.getObject(FireworkEffect.class, map, EXPLOSION.BUKKIT, true));
    }

    RunesCharge(NBTTagCompound tag) {
        super(tag);

        if (tag.hasKey(EXPLOSION.NBT)) {
            effect = RunesFirework.getEffect(tag.getCompound(EXPLOSION.NBT));
        }
    }
    ...
    void applyToItem(NBTTagCompound itemTag) {
        ...
        if (hasEffect()) {
            itemTag.set(EXPLOSION.NBT, RunesFirework.getExplosion(effect));
        }
    }
    ...
    boolean equalsCommon(RunesMeta meta) {
        ...
        if (meta instanceof RunesCharge) {
            RunesCharge that = (RunesCharge) meta;
    ...
    boolean notUncommon(RunesMeta meta) {
        return super.notUncommon(meta) && (meta instanceof RunesCharge || !hasChargeMeta());
    }
    ...
    int applyHash() {
        ...
        return hash != original ? RunesCharge.class.hashCode() ^ hash : hash;
    }

    @Override
    public RunesCharge clone() {
        return (RunesCharge) super.clone();
    }
